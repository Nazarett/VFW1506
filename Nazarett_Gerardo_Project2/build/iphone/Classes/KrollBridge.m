...
NSString * Nazarett_Gerrdo_project2$ModuleRequireFormat = @"(function(exports){"
...
@implementation Nazarett_Gerrdo_project2Object
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_nazarett_gerrdo_project2);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create Nazarett_Gerrdo_project2 global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "Nazarett_Gerrdo_project2" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_nazarett_gerrdo_project2 = [[Nazarett_Gerrdo_project2Object alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_nazarett_gerrdo_project2];
	
	NSString *_nazarett_gerrdo_project2NS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _nazarett_gerrdo_project2NS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_nazarett_gerrdo_project2 valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:Nazarett_Gerrdo_project2$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.nazarett_gerrdo_project2.kroll" 
