...
//Rare exceptions to indicate a bug in the _imagepractice code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_imagepractice = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_imagepractice]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.imagepractice.module"
